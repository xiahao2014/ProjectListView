...
import com.efunor.project_l.util.DataTools;
...
public class BreadcrumbLayout extends HorizontalScrollView {
    ...
    private OnClickListener mCrumbClickListener;
    ...
    public BreadcrumbLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        crumbStrip.setBackgroundColor(getResources().getColor(R.color.page_custom_bg));


        LayoutParams crumbStripLp = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
//        crumbStripLp.gravity = Gravity.LEFT;
//        crumbStripLp.leftMargin = 60;
    ...
    private void updateCrumbViews(final boolean requestLayout) {
        for (int i = 0; i < crumbStrip.getChildCount(); i++) {
            ...
            updateCrumbViewLayoutParams((LayoutParams) child.getLayoutParams());
            if (requestLayout) {
                child.requestLayout();
            }
        }
        ...
        if (selectedCrumb == crumb) {
            if (selectedCrumb != null) {
                ...
                if (crumb.getPosition() < crumbs.size() - 1) {
                    ...
                    for (int i = 0; i < crumbStrip.getChildCount(); i++) {
                        CrumbView beforeTextview = (CrumbView) crumbStrip.getChildAt(i);
                        if (crumb.getText().toString().equals(beforeTextview.mTextView.getText().toString())) {
                            beforeTextview.mTextView.setTextColor(getResources().getColor(R.color.head_custom_bg_normal));
                        } else {
                            beforeTextview.mTextView.setTextColor(getResources().getColor(R.color.text_custom_color_headline));
                        }
                    }
            ...
            if (crumb.getPosition() < crumbs.size() - 1) {
//                updaeTextView();
//                ((CrumbView) crumbStrip.getChildAt(crumb.getPosition())).mTextView.setTextColor(getResources().getColor(R.color.head_custom_bg_normal));
                for (int i = 0; i < crumbStrip.getChildCount(); i++) {
                    CrumbView beforeTextview = (CrumbView) crumbStrip.getChildAt(i);
                    if (crumb.getText().toString().equals(beforeTextview.mTextView.getText().toString())) {
                        beforeTextview.mTextView.setTextColor(getResources().getColor(R.color.head_custom_bg_normal));
    ...
    private CrumbView createCrumbView(Breadcrumb crumb) {
        ...
        updaeTextView();
        crumbView.mTextView.setTextColor(getResources().getColor(R.color.head_custom_bg_normal));


        if (mCrumbClickListener == null) {
            mCrumbClickListener = new OnClickListener() {
                @Override
    ...
    private LayoutParams createLayoutParamsForCrumbs() {
    ...
    private void updateCrumbViewLayoutParams(LayoutParams lp) {
    ...
    /**
     * Set the {@link OnBreadcrumbSelectedListener} that will
        ...
        if (position > 0) {
            ...
            animator.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    ((CrumbView) crumbStrip.getChildAt(0)).mTextView.setTextColor(getResources().getColor(R.color.head_custom_bg_normal));
                }
...
//                - (getWidth() / 2)
    ...
    class CrumbView extends LinearLayout {
        ...
        public CrumbView(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            setBackgroundColor(getResources().getColor(R.color.page_custom_bg));
        ...
        public void onMeasure(final int origWidthMeasureSpec, final int origHeightMeasureSpec) {
            ...
            if (mTextView != null) {
                ...
                int crumbTextMultiLineSize = res.getDimensionPixelSize(R.dimen.bottombar_textsize);
                float textSize = DataTools.dip2px(mContext, 18);
                ...
                final int curMaxLines = getMaxLines(mTextView);
    ...
    static int getMaxLines(TextView textView) {
        ...
        if (mMaximumField != null && mMaxModeField != null) {
            ...
            try {
                mMaximum = mMaximumField.getInt(textView); // Maximum value
                final int mMaxMode = mMaxModeField.getInt(textView); // Maximum mode value

                if (mMaxMode == 1) { // LINES is 1
//                    textView.setText(Integer.toString(mMaximum));
                }
